...
//!    - Part 2: Ambient Breakdown & Call-and-Response Development (Bars 48..95 - 48 bars):
//!      - Bars 48..55: Ambient Breakdown: drums completely drop out, spacious sustained Rhodes chords
//!        and deep wood bass pedal tones float in analog vinyl atmosphere.
//!      - Bars 56..63: Call-and-Response: High Rhodes calls motif; lower Rhodes/bass responds.
//!      - Bars 64..79: Gradual Groove Rebuild: drums re-enter softly, building dynamically into a rich climax.
//!      - Bars 80..87: Resolution into peaceful coda.
...
/// Renders a complete 96-bar Minimal Music arrangement with Ambient Breakdown Part 2 at 75 BPM (~5 minutes)
...
pub fn gen(init: std.process.Init) !lightmix.Wave(T) {
    ...
    // Continuous Vinyl Crackle Atmosphere across 96 bars with fading tail during final 8 bars
    ...
    // ========================================================
    // PART 2: AMBIENT BREAKDOWN & CALL-AND-RESPONSE (Bars 48..79)
    // ========================================================

    // Subsection A: Pure Ambient Breakdown (Bars 48..55 - 8 bars)
    // Drums drop out completely. Sustained chords and bass pedal tone.
    var bk_b: usize = 48;
    while (bk_b < 56) : (bk_b += 2) {
        try phrases._0007.load(T, synthesizers.wood_bass.WoodBass, utils.scale.Scale, &seq, bass_track, .{ .bar = bk_b, .beat = 0.0 }, VOLUME * 0.65);
        try phrases._0006.loadInstrument(T, synthesizers.rhodes.Rhodes, utils.scale.Scale, &seq, rhodes_chords, .{ .bar = bk_b, .beat = 0.0 }, VOLUME * 0.55);
    }

    // Subsection B: Call-and-Response Dialogue (Bars 56..63 - 8 bars)
    // High Rhodes calls motif (octave +1), lower Rhodes answers.
    const call_voice = &[_]utils.sequencer.Stagger.VoiceConfig(T){
        .{ .bar_offset = 0, .string_index = 0, .volume = 0.75, .octaves = 1 },
    };
    const response_voice = &[_]utils.sequencer.Stagger.VoiceConfig(T){
        .{ .bar_offset = 0, .string_index = 1, .volume = 0.70, .octaves = 0 },
    };
    var cr_b: usize = 56;
    while (cr_b < 64) : (cr_b += 4) {
        try phrases._0007.load(T, synthesizers.wood_bass.WoodBass, utils.scale.Scale, &seq, bass_track, .{ .bar = cr_b, .beat = 0.0 }, VOLUME * 0.75);
        try phrases._0007.load(T, synthesizers.wood_bass.WoodBass, utils.scale.Scale, &seq, bass_track, .{ .bar = cr_b + 2, .beat = 0.0 }, VOLUME * 0.75);
        try phrases._0006.loadInstrument(T, synthesizers.rhodes.Rhodes, utils.scale.Scale, &seq, rhodes_chords, .{ .bar = cr_b, .beat = 0.0 }, VOLUME * 0.60);
        try phrases._0006.loadInstrument(T, synthesizers.rhodes.Rhodes, utils.scale.Scale, &seq, rhodes_chords, .{ .bar = cr_b + 2, .beat = 0.0 }, VOLUME * 0.60);

        // Call (high register, bars 0..1 of 4-bar unit)
        try utils.sequencer.Stagger.scheduleCanon(T, utils.scale.Scale, synthesizers.rhodes.Rhodes, utils.scale.Scale, phrases._0005.phrase_data, &seq, theme_layers, .{ .bar = cr_b, .beat = 0.0 }, call_voice);
        // Response (middle register, bars 2..3 of 4-bar unit)
        try utils.sequencer.Stagger.scheduleCanon(T, utils.scale.Scale, synthesizers.rhodes.Rhodes, utils.scale.Scale, phrases._0005.phrase_data, &seq, theme_layers, .{ .bar = cr_b + 2, .beat = 0.0 }, response_voice);
    }

    // Subsection C: Dynamic Rebuild & Climax (Bars 64..79 - 16 bars)
    // Drums re-enter with swing hi-hat and kick, dynamic crescendo.
    var rb_b: usize = 64;
    while (rb_b < 80) : (rb_b += 2) {
        try phrases._0007.load(T, synthesizers.wood_bass.WoodBass, utils.scale.Scale, &seq, bass_track, .{ .bar = rb_b, .beat = 0.0 }, VOLUME * 0.85);
        try phrases._0006.loadInstrument(T, synthesizers.rhodes.Rhodes, utils.scale.Scale, &seq, rhodes_chords, .{ .bar = rb_b, .beat = 0.0 }, VOLUME * 0.75);

        for (0..2) |bar_sub| {
            const b = rb_b + bar_sub;
            try seq.add(kick_track, try createKickWave(allocator, SAMPLE_RATE, CHANNELS, VOLUME * 0.65), .{ .bar = b, .beat = 0.0 });
            try seq.add(kick_track, try createKickWave(allocator, SAMPLE_RATE, CHANNELS, VOLUME * 0.55), .{ .bar = b, .beat = 2.5 });
            for (0..4) |beat_idx| {
                const beat_f: f64 = @floatFromInt(beat_idx);
        ...
        try utils.sequencer.Stagger.scheduleCanon(T, utils.scale.Scale, synthesizers.rhodes.Rhodes, utils.scale.Scale, phrases._0005.phrase_data, &seq, theme_layers, .{ .bar = rb_b, .beat = 0.0 }, tri_layers);
    }
